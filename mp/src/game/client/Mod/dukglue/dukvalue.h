	...
	std::vector<char> serialize() const {
		...
		*((Type*)buff.data()) = mType;
		...
		case OBJECT:
		{
			...
			const char* json = duk_json_encode(mContext, -1);
			duk_pop(mContext);

			uint32_t len = Q_strlen(json);
			...
			*len_ptr = len;
			...
			char* out_ptr = (char*)(buff.data() + sizeof(Type) + sizeof(uint32_t));
			strncpy(out_ptr, json, len);  // note: this will NOT be null-terminated
			break;
		}
